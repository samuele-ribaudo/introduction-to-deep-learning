...
class MyPytorchModel(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.model = None
        self.device = hparams.get("device", torch.device("mps" if torch.backends.mps.is_available() else "cpu"))
        ...
        hidden_size = self.hparams["n_hidden"]
        mult = self.hparams["hidden_scale_factors"]

        if(hidden_layers == 2):
            self.fc1 = nn.Linear(self.hparams["input_size"], int(hidden_size * mult[0]))
            self.relu1 = nn.ReLU()
            self.fc2 = nn.Linear(int(hidden_size * mult[0]), int(hidden_size * mult[1]))
            self.relu2 = nn.ReLU()
            self.fc3 = nn.Linear(int(hidden_size * mult[1]), self.hparams["num_classes"])
            
            self.model = nn.Sequential(
                nn.BatchNorm1d(self.hparams["input_size"]),
                self.fc1, self.relu1, 
                nn.Dropout(self.hparams.get("dropout_prob", 0.3)),
                self.fc2, self.relu2, 
                nn.Dropout(self.hparams.get("dropout_prob", 0.3)),
                self.fc3)
            ...
            self.fc2 = nn.Linear(int(hidden_size * mult[0]), int(hidden_size * mult[1]))
            self.relu2 = nn.ReLU()
            self.fc3 = nn.Linear(int(hidden_size * mult[1]), int(hidden_size * mult[2]))
            self.relu3 = nn.ReLU()
            self.fc4 = nn.Linear(int(hidden_size * mult[2]), self.hparams["num_classes"])
